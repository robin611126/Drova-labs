...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-brand-dark/80 backdrop-blur-xl border-b border-white/5' : 'bg-transparent'
      }`}>
            ...
            <div className="ml-10 flex items-center space-x-6">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`text-sm font-medium transition-all duration-200 hover:text-brand-purple ${location.pathname === link.path ? 'text-white' : 'text-gray-400'
                    }`}
                >
                  {link.name}
                </Link>
...
const Hero = () => {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden pt-32 md:pt-40">
      ...
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-purple/20 rounded-full blur-[120px] pointer-events-none animate-pulse"></div>
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-brand-accent/10 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-white/5 border border-white/10 text-sm font-medium text-brand-accent mb-8 backdrop-blur-md shadow-[0_0_15px_rgba(6,182,212,0.15)]">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-accent opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-brand-accent"></span>
              </span>
              Next-Gen Ad Intelligence
            </div>

            <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight mb-8 font-heading leading-[1.05]">
              Scale Faster with <br className="hidden lg:block" />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-brand-purple to-brand-accent drop-shadow-[0_0_15px_rgba(124,58,237,0.3)]">AI Creativity</span>
            </h1>

            <p className="text-lg md:text-xl text-brand-text/90 max-w-xl mx-auto lg:mx-0 mb-10 leading-relaxed font-light">
              Stop burning cash on generic ads. Generate high-converting UGC, cinematic commercials, and AI avatars that <span className="text-white font-medium">outperform human production by 10x</span>.
            </p>

            <div className="flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start mb-16 lg:mb-24">
              <Link to="/contact" className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-brand-purple to-brand-purpleDark hover:from-brand-accent hover:to-brand-purple text-white rounded-xl font-bold transition-all shadow-[0_0_30px_rgba(124,58,237,0.4)] hover:shadow-[0_0_40px_rgba(124,58,237,0.6)] hover:scale-105 flex items-center justify-center gap-2">
                Launch Campaign <Zap size={18} fill="currentColor" />
              </Link>
              <Link to="/portfolio" className="w-full sm:w-auto px-8 py-4 bg-white/5 border border-white/10 hover:border-white/30 text-white hover:bg-white/10 rounded-xl font-bold transition-all relative overflow-hidden group">
                <div className="absolute inset-0 bg-white/5 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                <span className="relative">View Case Studies</span>
              </Link>
            </div>

            {/* Glass Stats Bar (Reference Layout) */}
            <div className="flex flex-col sm:flex-row items-center gap-8 lg:gap-12">
              {/* Stats Box */}
              <div className="flex items-center gap-8 px-8 py-4 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10">
                <div>
                  <div className="text-2xl font-bold text-white font-display">10x</div>
                  <div className="text-xs text-brand-text">ROI Avg</div>
                </div>
                <div className="w-px h-8 bg-white/10"></div>
                <div>
                  <div className="text-2xl font-bold text-white font-display">20k+</div>
                  <div className="text-xs text-brand-text">Auctions</div>
                </div>
                <div className="w-px h-8 bg-white/10"></div>
                <div>
                  <div className="text-2xl font-bold text-white font-display">7k+</div>
                  <div className="text-xs text-brand-text">Artists</div>
                </div>
              </div>
              ...
              <div className="text-center sm:text-left">
                <div className="flex items-center justify-center sm:justify-start -space-x-4 mb-2">
                  {[1, 2, 3, 4].map(i => (
                    <div key={i} className="w-10 h-10 rounded-full border-2 border-[#0a0b1e] bg-zinc-800 overflow-hidden">
                      <img src={`https://picsum.photos/100/100?random=${i}`} className="w-full h-full object-cover" alt="User" />
                    </div>
                  ))}
                </div>
                <div className="text-sm font-bold text-white">40k+ <span className="font-normal text-gray-400">Active Users</span></div>
              </div>
            </div>
          </motion.div>

          {/* Right Visuals - Hidden on small mobile, visible on large */}
          <div className="relative h-[600px] hidden md:block">
            {/* Abstract Background Blur */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-gradient-radial from-brand-purple/20 to-transparent blur-[60px] opacity-60"></div>

            {/* Floating Elements Composition */}
            <motion.div
              animate={{ y: [0, -20, 0] }}
              transition={{ repeat: Infinity, duration: 6, ease: "easeInOut" }}
              className="absolute top-10 right-10 z-10 max-w-sm w-full"
            >
              <div className="relative aspect-[9/16] rounded-[2rem] border-[6px] border-[#1A1A1D] bg-black overflow-hidden shadow-2xl shadow-brand-purple/30">
                {/* Mock Phone Screen */}
                <iframe
                  src="https://player.vimeo.com/video/1159353921?autoplay=1&loop=1&muted=1&background=1"
                ...
                {/* Overlay UI Elements */}
                <div className="absolute bottom-8 left-4 right-4 p-4 rounded-xl bg-white/10 backdrop-blur-md border border-white/20">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-brand-purple to-blue-500"></div>
                    <div className="text-xs font-bold">@drovalabs_ai</div>
                  </div>
            ...
            >
              <div className="p-5 rounded-2xl bg-[#0F0F11]/90 backdrop-blur-xl border border-white/10 shadow-2xl w-64">
                ...
                <div className="h-10 flex items-end gap-1">
                  {[40, 60, 45, 70, 90, 85, 100].map((h, i) => (
                    <div key={i} style={{ height: `${h}%` }} className="flex-1 bg-brand-purple rounded-sm opacity-80"></div>
                  ))}
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="absolute top-20 left-10 z-0 opacity-60 blur-[1px]"
            >
              <div className="p-4 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 w-48">
                <div className="flex gap-3 items-center">
                  <div className="p-2 bg-brand-accent/20 rounded-lg text-brand-accent"><Award size={18} /></div>
                  <div>
                    <div className="text-xs text-gray-400">Top Rated</div>
                    <div className="text-sm font-bold">AI Agency</div>
                  </div>
                </div>
              </div>
            </motion.div>
...
const Showcase = () => {
  const items = [
    { label: 'UGC', type: 'image', src: 'https://picsum.photos/400/700?random=21' },
    { label: 'Ad', type: 'video', src: 'https://player.vimeo.com/video/1158022423?autoplay=1&loop=1&muted=1&background=1' },
    { label: 'Avatar', type: 'image', src: 'https://picsum.photos/400/700?random=23' }
  ];
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-8">
          {items.map((item, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative aspect-[9/16] rounded-xl overflow-hidden cursor-pointer border border-white/10 bg-brand-card box-glow transition-all"
            >
              {item.type === 'video' ? (
                <iframe
                  src={item.src}
              ...
              <div className="absolute top-2 left-2 md:top-4 md:left-4 z-20">
                <span className="px-2 py-0.5 md:px-3 md:py-1 text-[8px] md:text-xs font-bold bg-black/60 backdrop-blur-md rounded-full text-white border border-white/10">
                  {item.label}
                </span>
              </div>
              ...
              {item.type !== 'video' && (
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-10">
                  <div className="w-8 h-8 md:w-16 md:h-16 rounded-full bg-brand-purple/90 backdrop-blur text-white flex items-center justify-center shadow-[0_0_20px_rgba(124,58,237,0.6)]">
                    <Play fill="currentColor" className="ml-0.5 md:ml-1 w-3 h-3 md:w-6 md:h-6" />
                  </div>
                </div>
              )}

              <div className="absolute bottom-0 left-0 right-0 p-2 md:p-6 bg-gradient-to-t from-black via-black/80 to-transparent pointer-events-none">
                <div className="flex flex-col md:flex-row justify-between items-end">
                  <div className="hidden md:block">
                    <h3 className="font-bold text-lg text-white">Campaign {index + 1}</h3>
                    <p className="text-sm text-gray-400">Tech Industry</p>
                  </div>
                  <div className="text-right w-full md:w-auto">
                    <div className="text-xs md:text-brand-accent font-bold text-white text-center md:text-right">4.2% CTR</div>
                  </div>
                </div>
              </div>
            </motion.div>
  ...
  return (
    <section className="py-32 bg-[#050505] relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full opacity-20 pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-brand-purple/20 rounded-full blur-[100px]"></div>
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-brand-accent/20 rounded-full blur-[100px]"></div>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <SectionHeading subtitle="The Shift" centered>Why Brands Are Switching</SectionHeading>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {problems.map((p, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative p-8 h-64 rounded-[2rem] bg-zinc-900/40 border border-white/5 hover:border-brand-purple/50 transition-all duration-500 overflow-hidden hover:bg-zinc-900/60"
            >
              {/* Hover Gradient */}
              ...
              <div className="relative z-10 h-full flex flex-col justify-between">
                <div className="flex justify-between items-start">
                  <div className="px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-medium line-through decoration-red-500/50">
                    {p.problem}
                  </div>
                  <div className="p-3 rounded-xl bg-white/5 text-gray-400 group-hover:text-brand-purple group-hover:bg-brand-purple/10 transition-colors">
                    <p.icon size={24} />
                  </div>
                </div>

                <div>
                  <div className="w-8 h-1 bg-gradient-to-r from-brand-purple to-transparent mb-4 rounded-full"></div>
                  <h3 className="text-2xl font-bold text-white group-hover:text-brand-accent transition-colors">{p.solution}</h3>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <div className="flex justify-center gap-4 mb-12 flex-wrap">
          {filters.map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-6 py-2 rounded-full border text-sm font-medium transition-all ${filter === f
